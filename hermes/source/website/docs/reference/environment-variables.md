...
For native Anthropic auth, Hermes prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. **OAuth against Anthropic requires a Claude Max plan** — Hermes routes as Claude Code, which draws from the Max plan's base allowance quota (the same pool used by the official Claude Code CLI). Extra usage credits are consumed only after the base allowance is exhausted. Without a Max plan, use an API key instead. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Max login.
...
| `WHATSAPP_ALLOWED_MEDIA_ROOT` | Existing absolute, non-symlink directory containing the regular files that the bridge may send through `/send-media`. The filesystem root is rejected; media sending fails closed when unset or invalid. |
