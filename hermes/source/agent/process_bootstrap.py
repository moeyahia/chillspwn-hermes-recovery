...
def _load_openai_cls() -> type:
    ...
    if _OPENAI_CLS_CACHE is None:
        ...
        # Monkeypatch openai.lib._parsing._responses.parse_response to prevent crash
        # when response.output is None on Responses API stream termination.
        try:
            import openai.lib._parsing._responses as parsing_responses
            original_parse_response = parsing_responses.parse_response

            def patched_parse_response(*args, **kwargs):
                response = kwargs.get("response") or (args[2] if len(args) > 2 else None)
                if response and getattr(response, "output", None) is None:
                    try:
                        response.output = []
            ...
            parsing_responses.parse_response = patched_parse_response
            import logging
            logging.getLogger(__name__).info(
                "Monkeypatched openai.lib._parsing._responses.parse_response for Responses API stream compatibility."
            )
        except Exception as patch_err:
            import logging
            logging.getLogger(__name__).warning(
                "Failed to patch openai.lib._parsing._responses.parse_response: %s",
                patch_err,
            )
