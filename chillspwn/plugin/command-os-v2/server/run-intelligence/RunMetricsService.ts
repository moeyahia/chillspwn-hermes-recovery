...
export class RunMetricsService {
  ...
  #compute(runId: string, computedAt: string): RunMetricsSnapshot {
    ...
    const sourceCounts: RunMetricsSourceCounts = {
      ...
      evidence: count(this.database, "SELECT COUNT(*) AS count FROM evidence WHERE run_id = ?", runId),
    ...
    const verifiedEvidence = count(this.database, "SELECT COUNT(*) AS count FROM evidence WHERE run_id = ? AND verification_state = 'verified'", runId);
    ...
    const coveredFindings = count(this.database, `
      ...
      WHERE f.run_id = ? AND EXISTS (
        ...
        WHERE fe.finding_id = f.id AND e.verification_state != 'rejected'
      )
    `, runId);
    const firstEvidence = this.database.prepare(`
      SELECT MIN(acquired_at) AS value FROM evidence WHERE run_id = ? AND verification_state != 'rejected'
