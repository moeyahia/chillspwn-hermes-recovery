...
export class ApprovedLegacyVaultProjectionService {
  ...
  async project(input: ApprovedLegacyVaultProjectionInput): Promise<ApprovedLegacyVaultProjectionResult> {
    ...
    const exportable = new Set(this.bridge.exportableNodeIds(input.connectionId));
    ...
    this.database.prepare(`
      INSERT INTO legacy_vault_projection_approvals (
        id, migration_id, reconciliation_hash, connection_id, approved_by,
    ...
    try {
      ...
      this.database.prepare(`
        ...
        SET status='completed', result_json=?, completed_at=? WHERE id=?
      `).run(canonical(result), completedAt, approvalId);
      this.appendAudit({
        approvalId,
      ...
      return { approvalId, projectedNodeIds: nodeIds, export: result };
    } catch (error) {
      this.database.prepare(`
        UPDATE legacy_vault_projection_approvals
