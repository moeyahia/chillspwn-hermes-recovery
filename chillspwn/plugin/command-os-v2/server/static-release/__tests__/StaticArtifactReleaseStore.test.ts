...
function fixture() {
  const workspace = mkdtempSync(join(tmpdir(), "command-os-v2-static-release-test-"));
  roots.push(workspace);
  const sourceDirectory = join(workspace, "command-os-v2-dist");
  const releaseRoot = join(workspace, "command-os-v2-release-root");
  ...
  test("rejects symlinks, special files, path escapes, legacy roots, and duplicate immutable IDs", () => {
    ...
    expectStaticError(
      () => new StaticArtifactReleaseStore({
        releaseRoot: join(workspace, "webapp", "command-os-v2-releases"),
      }),
