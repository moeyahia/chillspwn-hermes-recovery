...
function spawnServer(input: {
  ...
  const child = Bun.spawn([process.execPath, "run", "server/index.ts"], {
    ...
    env: {
      ...process.env,
      COMMAND_OS_V2_BIND: "127.0.0.1",
      COMMAND_OS_V2_PORT: String(input.port),
      COMMAND_OS_V2_DATABASE_PATH: input.databasePath,
      COMMAND_OS_V2_SCRIPT_SOURCE_ROOT: input.scriptSourceRoot,
      COMMAND_OS_V2_OBSIDIAN_VAULT_ROOT: input.vaultRoot,
      COMMAND_OS_V2_STATIC_RELEASE_ROOT: input.releaseRoot,
      COMMAND_OS_V2_SERVE_STATIC: "true",
      COMMAND_OS_V2_PREVIEW: "true",
      COMMAND_OS_V2_KILL_SWITCH: "false",
      COMMAND_OS_V2_TEST_RUN_CONTROL_RUNTIME: "false",
      COMMAND_OS_V2_SECURE_COOKIES: "false",
      COMMAND_OS_V2_OPERATOR_TOKEN: "isolated-static-handoff-test-token",
      COMMAND_OS_V2_OPERATOR_ID: "static-handoff-test-operator",
      COMMAND_OS_V2_UI_ORIGIN: `http://127.0.0.1:${input.port}`,
...
test("standalone V2 server pins an immutable static release across pointer changes and fails startup on tamper", async () => {
  const workspace = mkdtempSync(join(tmpdir(), "command-os-v2-static-server-integration-"));
  fixtureRoots.push(workspace);
  const sourceDirectory = join(workspace, "command-os-v2-built-dist");
  const releaseRoot = join(workspace, "command-os-v2-static-releases");
  const dataRoot = join(workspace, "command-os-v2-data");
  const databasePath = join(dataRoot, "command-os-v2-static-server.sqlite");
  const vaultRoot = join(workspace, "command-os-v2-obsidian-vault");
  const scriptSourceRoot = join(workspace, "command-os-v2-artifacts", "script-sources");
  ...
  expect(refusedProcess.stderr.text()).toContain("Command OS V2 failed to start");
