...
export function runStaticReleaseCli(args: readonly string[], options: CliOptions = {}): number {
  ...
  const root = required(values, "--root", environment.COMMAND_OS_V2_STATIC_RELEASE_ROOT);
  const store = new StaticArtifactReleaseStore({ releaseRoot: resolve(root) });
  let result: unknown;
  if (command === "stage") {
    result = store.stageRelease({
      releaseId: required(values, "--release-id", environment.COMMAND_OS_V2_STATIC_RELEASE_ID),
      sourceDirectory: resolve(values.get("--dist") ?? environment.COMMAND_OS_V2_DIST_ROOT ?? resolve(cwd, "dist")),
    });
  } else if (command === "verify") {
    result = store.verifyRelease(required(values, "--release-id", environment.COMMAND_OS_V2_STATIC_RELEASE_ID));
  } else if (command === "activate") {
    result = store.activateRelease(required(values, "--release-id", environment.COMMAND_OS_V2_STATIC_RELEASE_ID));
  } else if (command === "pin") {
