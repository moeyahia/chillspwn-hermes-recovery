...
const MANIFEST_SCHEMA = "command-os-v2.static-artifact-manifest.v1" as const;
const POINTER_SCHEMA = "command-os-v2.static-artifact-pointer.v1" as const;
...
function v2OwnedAbsolutePath(path: string, label: string): string {
  ...
  if (!segments.some((segment) => V2_NAMESPACE.test(segment))) {
    throw new StaticReleaseError("invalid_v2_release_path", `${label} must include a command-os-v2 namespace segment`);
  }
 ...
 * command-os-v2 release root. It does not restart a process, change a reverse
...
export class StaticArtifactReleaseStore {
  ...
  constructor(options: StaticArtifactReleaseStoreOptions) {
    this.releaseRoot = v2OwnedAbsolutePath(options.releaseRoot, "COMMAND_OS_V2_STATIC_RELEASE_ROOT");
  ...
  stageRelease(input: { readonly releaseId: string; readonly sourceDirectory: string }): VerifiedStaticRelease {
    ...
    const source = v2OwnedAbsolutePath(input.sourceDirectory, "Command OS V2 static artifact source");
    requireDirectory(source, "Command OS V2 static artifact source");
  ...
  #ensureLayout(): void {
    ensureDirectory(this.releaseRoot, "Command OS V2 static release root");
    ensureDirectory(this.releasesDirectory, "Command OS V2 immutable releases directory");
    ensureDirectory(this.stateDirectory, "Command OS V2 static release state directory");
  }

  #requireLayout(): void {
    requireDirectory(this.releaseRoot, "Command OS V2 static release root");
    requireDirectory(this.releasesDirectory, "Command OS V2 immutable releases directory");
    requireDirectory(this.stateDirectory, "Command OS V2 static release state directory");
  }
