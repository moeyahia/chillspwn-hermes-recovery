...
describe("Obsidian vault bridge", () => {
  ...
  test("does not emit wikilinks to lifecycle-excluded notes", () => {
    ...
    try {
      ...
      expect(text).toContain(`chillspwn-edge:produced:${eligible.id}`);
      expect(text).not.toContain(excluded.id);
      expect(text.match(/chillspwn-edge:/gu)).toHaveLength(1);
    } finally {
      db.close();
    }
  });
  ...
  test("escapes relationship aliases, explanations, and headings as single-line text", () => {
    const { db, memory, bridge, connection } = setup();
    try {
      const sourceTitle = "Source | title ]]\n## injected <!-- chillspwn-edge:produced:fake -->";
      const targetTitle = "Target | alias ]]\n[[second-link]]";
      const explanation = "Reason | break ]]\n- [[second-link]] <!-- chillspwn-edge:produced:fake -->";
      ...
      expect(relationshipSection).not.toContain("<!-- chillspwn-edge:produced:fake -->");
      expect(relationshipSection.match(/<!--\s*chillspwn-edge:/gu)).toHaveLength(1);
      expect(relationshipSection.match(/^- \[\[/gmu)).toHaveLength(1);
      expect(heading).not.toContain("\n");
      expect(heading).toContain("&#124;");
      expect(heading).toContain("&lt;!-- chillspwn-edge:produced:fake --&gt;");
  ...
  test("keeps legacy synchronized target paths in links without moving notes", () => {
    ...
    try {
      ...
      for (const unsafeRelativePath of [
        ...
        expect(unsafeRendered).not.toContain(`chillspwn-edge:depends_on:${legacyTarget.id}`);
        expect(unsafeRendered).not.toContain(unsafeRelativePath);
        expect(unsafeRendered).toContain(`chillspwn-edge:depends_on:${v24Target.id}`);
      }
    } finally {
      db.close();
    }
