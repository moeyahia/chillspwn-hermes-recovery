    ...
    try {
      ...
      if (issues.length < MAX_ISSUES) issues.push({
        ...
        message: isPermissionError(error)
          ? "Command OS cannot read this managed Vault directory. Restore read and traverse permission, then retry recovery."
  ...
  run(input: {
    ...
    const quarantineOrDiagnose = (
      ...
      if (isPermissionError(error)) {
        counts.errors += 1;
        const message = "Command OS could not read this managed Vault note. Restore file read permission, confirm the path has not been replaced, and retry.";
