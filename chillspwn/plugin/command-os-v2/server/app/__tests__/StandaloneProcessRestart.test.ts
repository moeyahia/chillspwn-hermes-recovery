...
const DISPOSABLE_ROOT = join(tmpdir(), "chillspwn-command-os-v2-e2e-data");
const LEGACY_HEALTH_URL = process.env.COMMAND_OS_V2_TEST_LEGACY_HEALTH_URL?.trim()
...
function startServer(input: {
  ...
  const child = spawn(process.execPath, ["run", "server/index.ts"], {
    ...
    env: {
      ...
      COMMAND_OS_V2_SCRIPT_SOURCE_ROOT: input.scriptRoot,
      COMMAND_OS_V2_OPERATOR_TOKEN: input.operatorToken,
      COMMAND_OS_V2_OPERATOR_ID: "operator:process-boundary-restart",
      ...
      COMMAND_OS_V2_TEST_RUN_CONTROL_RUNTIME: input.recoveryRuntime ? "true" : "false",
      COMMAND_OS_V2_TEST_RUN_CONTROL_SCHEDULER: input.recoveryRuntime ? "true" : "false",
      COMMAND_OS_V2_E2E_RUN_ID: input.runId ?? "process-boundary-not-started",
    },
...
describe("standalone V2 process-boundary durability", () => {
  test("recovers one interrupted run exactly once and preserves its active Obsidian Vault", async () => {
    ...
    const databasePath = join(directory, "command-os-v2.sqlite");
