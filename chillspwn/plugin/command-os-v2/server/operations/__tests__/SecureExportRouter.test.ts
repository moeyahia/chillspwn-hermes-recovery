...
function seed(
  ...
  mkdirSync(policy.resolveRelative(vaultPath, ".chillspwn/attachments", true), { recursive: true });
  database.prepare(`
  ...
  policy.atomicWriteBytes(vaultPath, `.chillspwn/attachments/${goodHash}`, goodBody);
  ...
  policy.atomicWriteBytes(vaultPath, `.chillspwn/attachments/${sizeHash}`, sizeBody);
  artifact.run(
  ...
  symlinkSync(outside, policy.resolveRelative(vaultPath, `.chillspwn/attachments/${symlinkHash}`));
  artifact.run(
    "artifact-symlink",
    "mission-a",
    "run-a",
...
async function application() {
  ...
  const database = createDatabaseConnection({ filename: join(directory, "command-os.sqlite") });
...
describe("canonical secure artifact and export delivery", () => {
  test("delivers only verified vault attachments with inert server-derived headers and an immutable audit", async () => {
    ...
    try {
      ...
      expect(response.headers.get("content-disposition")).toBe(
        'attachment; filename="chillspwn-artifact-artifact-good.bin"',
      );
    ...
    try {
      ...
      expect(database.prepare(`
        ...
        WHERE resource_id IN (
          'artifact-unlinked', 'artifact-unverified',
  ...
  test("exports a bounded redacted evidence metadata bundle and audits the export", async () => {
    ...
    try {
      ...
      expect(response.status).toBe(200);
      expect(response.headers.get("content-disposition")).toBe(
        'attachment; filename="chillspwn-run-a-evidence.json"',
      );
      expect(response.headers.get("content-security-policy")).toBe("sandbox");
  ...
  test("requires restricted access and exports only a bounded redacted run-scoped audit subset", async () => {
    ...
    try {
      ...
      expect(response.status).toBe(200);
      expect(response.headers.get("content-disposition")).toBe(
        'attachment; filename="chillspwn-run-a-audit.json"',
      );
      const exported = await response.json() as any;
      expect(exported).toMatchObject({
        schemaVersion: "2.4",
