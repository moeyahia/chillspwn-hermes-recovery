...
describe("Command OS V2 contract", () => {
  ...
  test("accounts for implemented runtime boundaries that are intentionally not mounted", () => {
    const mounted = new Set(COMMAND_OS_V2_ENDPOINTS.map((endpoint) => `${endpoint.method}:${endpoint.path}`));
    const deferred = COMMAND_OS_V2_DEFERRED_ENDPOINTS
      .map((endpoint) => `${endpoint.method}:${endpoint.path}`)
      .sort();
    expect(deferred).toEqual(deferredImplementationRouteIds());
    expect(deferred.every((identity) => !mounted.has(identity))).toBe(true);
    expect(COMMAND_OS_V2_DEFERRED_ENDPOINTS.every((endpoint) =>
  ...
  test("emits a valid OpenAPI 3.1 document with shared errors", () => {
    ...
    expect(document["x-command-os-deferred-operations"]
