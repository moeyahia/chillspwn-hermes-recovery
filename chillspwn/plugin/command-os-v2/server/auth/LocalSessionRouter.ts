  ...
  router.post("/api/v2/auth/session", (request: Request, response: Response) => {
    ...
    if (!options.auth) {
      sendV2Error(response, traceId, {
        ...
        humanMessage: "This isolated Command OS V2 preview is not ready for sign-in.",
        retryable: false,
        category: "authentication_missing",
        remediation: "Set COMMAND_OS_V2_OPERATOR_TOKEN to a private value of at least 24 bytes and restart only V2.",
      });
      return;
    }
    ...
    if (!token || !options.auth.verifyOperatorToken(token)) {
      sendV2Error(response, traceId, {
        status: 401,
        code: "command_os_authentication_failed",
        message: "The supplied local operator token is invalid",
        humanMessage: "The local operator token was not accepted.",
        retryable: false,
        category: "authentication_missing",
        remediation: "Use the private token configured for this isolated Command OS V2 process.",
      });
      return;
    }
