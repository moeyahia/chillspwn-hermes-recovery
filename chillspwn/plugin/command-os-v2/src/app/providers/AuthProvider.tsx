...
function LoginSurface({
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!operatorToken) {
      setError(new Error("Enter the private local operator token configured for this V2 preview."));
      return;
    }
    setSubmitting(true);
    setError(undefined);
  ...
  return (
    <main className="os-auth-shell" aria-labelledby="command-os-sign-in-title">
      <section className="os-auth-panel">
        <img src={assetUrl("Logo.svg")} alt="" className="os-auth-logo" />
        <p className="os-eyebrow">CHILLSPWN // COMMAND OS</p>
        <h1 id="command-os-sign-in-title">Enter the isolated V2 preview</h1>
        <p>Authenticate locally before mission, evidence, memory, or runtime data is loaded. The token is exchanged for an HttpOnly, same-site V2 session and is not stored in browser storage.</p>
        {!configured ? (
          <div className="os-validation-summary" role="alert">
            <strong>Local authentication is not configured</strong>
            <p>Set <code>COMMAND_OS_V2_OPERATOR_TOKEN</code> to a private value of at least 24 bytes, then restart only the V2 process. The legacy application remains unaffected.</p>
          </div>
        ) : (
          <form onSubmit={submit} className="os-auth-form">
            <label htmlFor="command-os-operator-token">Local operator token</label>
            <input
              id="command-os-operator-token"
              type="password"
              value={operatorToken}
              ...
              autoFocus
            />
            <Button type="submit" disabled={submitting}>{submitting ? "Signing in…" : "Sign in to Command OS"}</Button>
          </form>
        )}
        {error && <ErrorPanel
          ...
          error={error}
        />}
        <small>Authorized security operations only · V2.4 parallel preview</small>
      </section>
    </main>
  );
}
...
export function AuthProvider({ children }: { readonly children: ReactNode }) {
  ...
  if (error) {
    return <main className="os-auth-shell"><section className="os-auth-panel"><ErrorPanel title="Command OS authentication readiness is unavailable" error={error} onRetry={() => void refresh()} /></section></main>;
  }
  if (!session) return <main className="os-auth-shell"><p role="status">Verifying isolated V2 session…</p></main>;
