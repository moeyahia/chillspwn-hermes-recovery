...
const sections: ManualSection[] = [
  {
    ...
    title: "How ChillsPwn organizes work",
    ...
    content: <>
      <p>Command OS is mission-first, not chat-first. A conversation can explain or control part of a mission, but it is not the owner of execution state.</p>
  ...
  },
  {
    ...
    summary: "Define the complete operating contract once; ChillsPwn plans, delegates, executes, recovers, validates, and reports inside it.",
  ...
  },
  {
    ...
    summary: "ChillsPwn explains one bounded step, recommends it, waits for your choice, interprets the result, records evidence, and advances.",
  ...
  },
  {
    ...
    content: <>
      ...
      <p>The deployed page has no Autonomous contract queue or administrative approve/reject form. The legacy <code>/approvals</code> address redirects here. Autonomous work must not wait here for routine approval.</p>
    </>,
  },
  {
    id: "intelligence",
  ...
  },
  {
    ...
    content: <>
      ...
      <Callout title="The vault path is on the ChillsPwn server">
  ...
  },
  {
    ...
    content: <>
      ...
      <p>Legacy names such as <code>waiting_input</code> or <code>awaiting_action_approval</code> are compatibility values and should not appear as new journey choices.</p>
    </>,
  },
  {
    id: "safety-privacy",
    ...
    content: <>
      <ul>
        <li>Use ChillsPwn only for work you are authorized to perform. Keep the mission’s allowed and prohibited target boundary precise and current.</li>
  ...
  },
  {
    ...
    keywords: "limitations unavailable missing cannot engagement manager new run amendment agent assignment upload download settings authenticate install mcp legacy pagination",
    content: <>
      <ManualTable headings={["Not currently exposed", "What to do today"]} rows={[
        ...
        ["Legacy desktop applications", "They are not active Command OS functions. Legacy routes redirect or remain read-only compatibility data."],
      ]} />
      <Callout title="Why the manual states limitations">
        <p>Showing an unavailable control as if it worked would be dangerous in an authorized operations product. A missing button is a product gap, not implied permission to bypass the runtime through a shell or hidden legacy route.</p>
      </Callout>
    </>,
  },
  {
    ...
    content: <>
      <ManualTable headings={["Symptom", "Check", "Safe response"]} rows={[
        ...
        ["Overview contains many old blocked items", "Check mission timestamps and legacy/import provenance.", "Imported nonterminal history may be deliberately blocked for review. Do not resume every historical item blindly."],
  ...
  },
  {
    ...
    summary: "Use this map to open every current Command OS product surface directly.",
...
export default function UserManualPage() {
  ...
  return (
    <div className="os-page manual-page">
      <PageHeader
        eyebrow="Command OS V2.4 live"
        title="ChillsPwn User Manual"
        ...
        <div className="manual-content">
          {visibleSections.length === 0 ? <section className="manual-no-results" role="status"><h2>No matching section</h2><p>Try a broader term such as mission, Guided, evidence, memory, recovery, or System.</p><button type="button" className="os-button os-button--secondary" onClick={() => setQuery("")}>Clear search</button></section> : visibleSections.map((section) => (
            <article className="manual-section" id={section.id} key={section.id} aria-labelledby={`${section.id}-title`}>
              ...
              <a className="manual-back-to-top" href="#command-os-content">Back to top</a>
            </article>
          ))}
        </div>
      </div>
