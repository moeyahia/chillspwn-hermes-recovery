...
function EventsView({ filters }: { filters: ReturnType<typeof useUrlFilters> }) {
  ...
  return <>
    ...
    <QueryBoundary data={query.data?.items} error={query.error} isLoading={query.isLoading} onRetry={query.refresh} emptyTitle="No operational events" emptyDescription="The append-only event stream has no records in this filter scope.">{(items) => <>
      <ol className="os-semantic-feed">{items.map((event) => <li key={event.id}><div className="os-feed-marker" aria-hidden="true" /><article><header><div><strong>{event.summary}</strong><span>{event.eventType}</span></div><time dateTime={event.occurredAt}>{formatTime(event.occurredAt)}</time></header><div className="os-feed-meta"><StatusPill status={event.journey ?? "system"} />{event.mission && <span>{event.mission.name}</span>}{event.actor.id && <span>{event.actor.type}: {event.actor.id}</span>}{event.correlation.traceId && <button type="button" onClick={() => filters.set({ view: "traces", traceId: event.correlation.traceId ?? undefined, cursor: undefined, recordCursor: undefined })}>Trace {event.correlation.traceId.slice(0, 12)}</button>}</div><JsonDetails label="Structured event payload" value={{ payload: event.payload, correlation: event.correlation, redaction: event.redaction }} /></article></li>)}</ol>
