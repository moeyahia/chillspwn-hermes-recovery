...
function EvidenceTable({ items, selectedId }: { items: EvidenceRecord[]; selectedId?: string }) {
  return <div className="os-table-wrap"><table className="os-data-table"><thead><tr><th>Evidence</th><th>Mission</th><th>Type</th><th>Verification</th><th>Acquired</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className={item.id === selectedId ? "is-selected" : undefined}><th scope="row"><AppLink href={evidenceDetailHref(item.id)}>{item.summary || item.id}</AppLink><small className="os-mono">{item.contentHash.slice(0, 12)}…</small></th><td><MissionRelation mission={item.mission} /></td><td>{item.evidenceType}</td><td><StatusPill status={item.verificationState} /></td><td>{formatTime(item.acquiredAt)}</td></tr>)}</tbody></table></div>;
}
