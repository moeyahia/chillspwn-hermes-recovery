...
export function createArtifactDeliveryFixture(instanceId: string): ArtifactDeliveryFixture {
  ...
  const content = Buffer.from(
    `ChillsPwn Command OS V2 verified artifact delivery fixture\nnamespace=${namespace}\n`,
    "utf8",
  );
  ...
  try {
    ...
    vaultPolicy.atomicWriteBytes(
      ...
      `.chillspwn/attachments/${contentHash}`,
      content,
    );
    ...
    vaultPolicy.atomicWriteBytes(
      connection.vaultPath,
      `.chillspwn/attachments/${mismatchHash}`,
    ...
    inImmediateTransaction(database, () => {
      const insertMission = database.prepare(`
        INSERT INTO missions (
          ...
          'e2e-local-operator', ?, ?, 'command_os_v2')
      `);
      insertMission.run(
        missionId,
