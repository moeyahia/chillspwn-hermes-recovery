...
function validateReleaseContract(
  ...
  validateExactReleaseValue(environment, "COMMAND_OS_V2_E2E_REQUIRE_API", "1", violations);
  validateExactReleaseValue(environment, "COMMAND_OS_V2_E2E_ENFORCE_MANIFEST", "1", violations);
  validateExactReleaseValue(
    environment,
    "COMMAND_OS_V2_E2E_SERVER_MODE",
    PLAYWRIGHT_MANAGED_STATIC_SERVER_MODE,
    violations,
  );
  validateExactReleaseValue(environment, "COMMAND_OS_V2_E2E_EXTERNAL_SERVERS", "false", violations);

  if (!configuredValue(environment, "COMMAND_OS_V2_E2E_BASE_URL")) {
    violations.push("COMMAND_OS_V2_E2E_BASE_URL must be explicitly set for the release profile");
  }
  if (!configuredValue(environment, "COMMAND_OS_V2_E2E_API_URL")) {
    violations.push("COMMAND_OS_V2_E2E_API_URL must be explicitly set for the release profile");
  }

  const parsedBaseURL = parsedUrl(baseURL, "COMMAND_OS_V2_E2E_BASE_URL", violations);
  const parsedApiURL = parsedUrl(apiURL, "COMMAND_OS_V2_E2E_API_URL", violations);
  validateReleaseUrl(parsedBaseURL, "COMMAND_OS_V2_E2E_BASE_URL", violations);
  validateReleaseUrl(parsedApiURL, "COMMAND_OS_V2_E2E_API_URL", violations);
  if (parsedBaseURL && parsedApiURL && parsedBaseURL.origin !== parsedApiURL.origin) {
    violations.push(
      `COMMAND_OS_V2_E2E_BASE_URL and COMMAND_OS_V2_E2E_API_URL must have the same origin; received ${parsedBaseURL.origin} and ${parsedApiURL.origin}`,
    );
  }
...
export function parseE2EProfile(environment: E2EEnvironment = process.env): ParsedE2EProfile {
  const profile = configuredValue(environment, "COMMAND_OS_V2_E2E_PROFILE") ?? "development";
  if (profile !== "development" && profile !== "release" && profile !== "degraded") {
    throw new Error(
      `COMMAND_OS_V2_E2E_PROFILE must be development, release, or degraded; received ${profile}`,
    );
  }

  if (profile !== "degraded" && environment.COMMAND_OS_V2_E2E_REQUIRE_API === "0") {
    throw new Error("Required V2 API auditing can be relaxed only by the explicit degraded profile");
  }

  const baseURL = configuredValue(environment, "COMMAND_OS_V2_E2E_BASE_URL") ?? DEFAULT_BASE_URL;
  const apiURL = configuredValue(environment, "COMMAND_OS_V2_E2E_API_URL") ?? DEFAULT_API_URL;
  if (profile === "release") validateReleaseContract(environment, baseURL, apiURL);

  return Object.freeze({
    profile,
    requireApi: profile !== "degraded",
    enforceManifest: profile === "release" || configuredValue(environment, "COMMAND_OS_V2_E2E_ENFORCE_MANIFEST") === "1",
    serverMode: configuredValue(environment, "COMMAND_OS_V2_E2E_SERVER_MODE"),
    externalServers: configuredValue(environment, "COMMAND_OS_V2_E2E_EXTERNAL_SERVERS") === "true",
    baseURL,
    apiURL,
