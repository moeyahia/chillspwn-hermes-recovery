...
export function e2eAuditProfile(environment: NodeJS.ProcessEnv = process.env): E2EAuditProfile {
  ...
  if (profile !== "development" && profile !== "release" && profile !== "degraded") {
    throw new Error(`COMMAND_OS_V2_E2E_PROFILE must be development, release, or degraded; received ${profile}`);
  }
...
export class BrowserAuditSession {
  ...
  private constructor(private readonly context: BrowserContext, options: BrowserAuditOptions = {}) {
    ...
    this.expectedOrigin = new URL(
      process.env.COMMAND_OS_V2_E2E_BASE_URL ?? "http://127.0.0.1:43140",
    ).origin;
  ...
  async closeAuditedPage(page: Page): Promise<void> {
    ...
    const exactOpenStreams = [...this.activeRequests.entries()]
      ...
      .map(([request, active]) => ({ request, url: active.url }));
    ...
    try {
      ...
      // exact request identities that were open before that call are resolved.
      for (const stream of exactOpenStreams) {
        if (!this.activeRequests.has(stream.request)) continue;
        this.pageCloseResolvedRequests.add(stream.request);
        this.activeRequests.delete(stream.request);
        this.eventStreamRequests.delete(stream.request);
        this.pageCloseEventStreamReceipts.push({ page, url: stream.url });
      }
    } finally {
