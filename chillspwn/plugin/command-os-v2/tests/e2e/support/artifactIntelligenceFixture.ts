...
export function createArtifactIntelligenceFixture(instanceId: string): ArtifactIntelligenceFixture {
  ...
  try {
    inImmediateTransaction(database, () => {
      ...
      `).run(missionId, JSON.stringify({ environment: "authorized_local_fixture", controlPlane: "command_os_v2" }), JSON.stringify(["Retain immutable source and capture provenance"]), ACTOR.id, NOW, NOW);
      database.prepare(`
