...
test.describe(`${TEST_ID} production rendering contract`, () => {
  test("Command Center renders the light technology operations system", async ({ page }, testInfo) => {
    ...
    const presentation = await page.evaluate(() => {
      const application = document.querySelector<HTMLElement>(".command-os");
    ...
    expect(presentation).toEqual({
      documentClass: "",
      bodyClass: "",
      bodyBackground: "rgb(237, 242, 241)",
      colorScheme: "light",
      canvas: "#edf2f1",
  ...
  test("Second Brain graph uses the light evidence-topology canvas", async ({ page }, testInfo) => {
    ...
    await expect(page.locator(".brain-graph-workspace")).toHaveCSS("background-color", "rgb(248, 250, 249)");
