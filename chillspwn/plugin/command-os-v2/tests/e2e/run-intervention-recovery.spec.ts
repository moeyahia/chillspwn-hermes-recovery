...
test(`${TEST_CONTROL_PLANE} refuses a legacy-owned run without creating V2 lease, event, checkpoint, or audit state`, async ({ page }, testInfo) => {
  ...
  expect(await response.json()).toMatchObject({
    error: {
      ...
      humanMessage: "This run belongs to another control plane and Command OS V2 refused to mutate it.",
  ...
  await expect(alert).toContainText("This run belongs to another control plane and Command OS V2 refused to mutate it.");
...
test(`${TEST_PAUSE_RESUME} pauses and resumes from the exact durable checkpoint with idempotent replay and refresh persistence`, async ({ page }, testInfo) => {
  ...
  expect(afterPause.run).toMatchObject({ status: "blocked", controlPlane: "command_os_v2", leaseOwner: null });
