...
describe("automatic E2E browser-audit boundary", () => {
  ...
  test("strict profiles are the default and relaxation requires explicit degraded mode", () => {
    ...
    expect(e2eAuditProfile({ COMMAND_OS_V2_E2E_PROFILE: "development" })).toBe("development");
    expect(e2eAuditProfile({ COMMAND_OS_V2_E2E_PROFILE: "release", COMMAND_OS_V2_E2E_REQUIRE_API: "1" })).toBe("release");
    expect(e2eAuditProfile({ COMMAND_OS_V2_E2E_PROFILE: "degraded", COMMAND_OS_V2_E2E_REQUIRE_API: "0" })).toBe("degraded");
    expect(() => e2eAuditProfile({ COMMAND_OS_V2_E2E_PROFILE: "smoke" })).toThrow(
