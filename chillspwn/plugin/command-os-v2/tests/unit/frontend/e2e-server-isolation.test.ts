...
describe("standalone browser-server isolation", () => {
  ...
  test("uses invocation-owned ports and authentication state", () => {
    expect(playwrightConfig).toContain("COMMAND_OS_V2_PORT: localApiPort");
    expect(playwrightConfig).toContain("--port ${localUiPort}");
    expect(playwrightConfig).toContain("COMMAND_OS_V2_API_ORIGIN: localApiServerUrl.origin");
    expect(playwrightConfig).not.toContain('COMMAND_OS_V2_PORT: "43141"');
    expect(environment).toContain("`auth-state-${E2E_RUN_ID}.json`");
    expect(environment).not.toContain("chillspwn-command-os-v2-e2e-auth-state.json");
  });
});
