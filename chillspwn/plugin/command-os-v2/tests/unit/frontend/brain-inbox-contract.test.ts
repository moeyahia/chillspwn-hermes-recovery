...
beforeEach(() => {
  ...
  Object.defineProperty(globalThis, "document", {
    configurable: true,
    value: { cookie: "chillspwn_command_os_v2_csrf=csrf-proof" },
  });
...
describe("Memory Inbox mutation contract", () => {
  test("sends opaque cursors with the active Brain search and exact-run scope", async () => {
    ...
    const nodeRequest = new URL(calls[0]!.path, "http://command-os-v2.local");
    ...
    const candidateRequest = new URL(calls[1]!.path, "http://command-os-v2.local");
  ...
  test("keeps ordinary rejection distinct from reject-and-do-not-relearn", async () => {
    ...
    expect(calls.every((call) => new Headers(call.init?.headers).get("X-Command-OS-V2-CSRF") === "csrf-proof"))
      .toBe(true);
  });
