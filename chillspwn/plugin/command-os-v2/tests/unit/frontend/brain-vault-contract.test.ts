...
beforeEach(() => {
  ...
  Object.defineProperty(globalThis, "document", {
    configurable: true,
    value: { cookie: "chillspwn_command_os_v2_csrf=vault-csrf-proof" },
  });
...
describe("Brain Vault presentation contract", () => {
  test("uses the mounted mutation for candidate and connected round-trip checks", async () => {
    ...
    expect(requests.every((item) => new Headers(item.init?.headers).get("X-Command-OS-V2-CSRF") === "vault-csrf-proof"))
      .toBe(true);
  });
