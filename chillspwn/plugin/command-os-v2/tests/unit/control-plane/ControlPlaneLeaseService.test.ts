...
function fixture() {
  const directory = mkdtempSync(join(tmpdir(), "command-os-control-plane-"));
...
describe("server-enforced control-plane leases", () => {
  test("stores only a token digest and authorizes the exact owner and plane", () => {
    ...
    try {
      ...
      expect(acquired.lease.controlPlane).toBe("command_os_v2");
  ...
  test("heartbeats idempotently, releases with fencing, and permits expired takeover", () => {
    ...
    try {
      ...
      service.acquire({
        runId: "run-v2", controlPlane: "command_os_v2", leaseOwner: "worker-v2-a",
      ...
      const takeover = service.acquire({
        runId: "run-v2", controlPlane: "command_os_v2", leaseOwner: "worker-v2-b",
