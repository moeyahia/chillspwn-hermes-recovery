...
describe("RunIntelligenceRouter", () => {
  ...
  test("keeps deterministic metrics ownership-only while lease-fencing attack-attempt mutations", async () => {
    ...
    harness.database.prepare("UPDATE missions SET control_plane = 'command_os_v2' WHERE id = ?").run(MISSION_ID);
    harness.database.prepare("UPDATE runs SET control_plane = 'command_os_v2' WHERE id = ?").run(RUN_ID);
