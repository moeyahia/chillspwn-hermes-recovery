...
export interface ReleaseAttestation {
  readonly schemaVersion: "command-os-v2.release-attestation.v1";
...
export function buildReleaseAttestation(input: {
  ...
  return {
    schemaVersion: "command-os-v2.release-attestation.v1",
    ...
    criticalHashes: {
      ...
      logo: criticalFileHash(resolve(input.applicationRoot, "public/Logo.svg")),
...
async function main(): Promise<void> {
  ...
  const output = outputFlag >= 0 && process.argv[outputFlag + 1]
    ...
    : resolve(applicationRoot, "test-results/attestations/command-os-v2-release-attestation.json");
