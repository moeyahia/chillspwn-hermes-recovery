...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [contextualRun, setContextualRun] = useState<RuntimeRun>();
  ...
  useEffect(() => {
    if (!open || (!routeContext.runId && !routeContext.missionId)) {
      setContextualRun(undefined);
      return;
    }
    const controller = new AbortController();
    setContextualRun(undefined);
    const load = routeContext.runId
      ? runtimeV2Api.run(routeContext.runId, controller.signal).then((snapshot) => snapshot.run)
      : runtimeV2Api.mission(routeContext.missionId!, controller.signal).then((snapshot) =>
          snapshot.runs.find(nonTerminal) ?? snapshot.runs[0]);
    void load.then((run) => { if (!controller.signal.aborted) setContextualRun(run); }).catch(() => {
      if (!controller.signal.aborted) setContextualRun(undefined);
    });
    return () => controller.abort();
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      void Promise.allSettled([
        ...
        setRemoteMatches({
          missions: missions.status === "fulfilled" ? missions.value.items : data.missions,
          runs: runs.status === "fulfilled" ? runs.value.items : data.runs,
          decisions: decisions.status === "fulfilled" ? decisions.value.items : data.decisions,
          agents: agents.status === "fulfilled" ? agents.value.items : data.agents,
        });
  ...
  async function controlRun(event: FormEvent): Promise<void> {
    ...
    try {
      ...
      setStatusMessage(`${editor.command === "cancel" ? "Cancelled" : editor.command === "pause" ? "Paused" : "Resumed"} ${updated.missionName}.`);
      resetEditor();
    } catch (error) {
      ...
      <div ref={dialogRef} className="os-palette" role="dialog" aria-modal="true" aria-labelledby="command-palette-title" onKeyDown={trapFocus}>
        ...
        {editor?.kind === "run-control" ? (
          <form className="os-palette-editor" onSubmit={controlRun}>
            ...
            <div className="os-palette-editor-actions"><button type="button" onClick={resetEditor}>Back</button><button className={editor.command === "cancel" ? "is-danger" : "is-primary"} disabled={mutationPending || reason.trim().length < 2 || (editor.command === "cancel" && !cancelConfirmed)}>{mutationPending ? "Applying…" : `Confirm ${editor.command}`}</button></div>
          </form>
          ...
          <form className="os-palette-editor" onSubmit={createMemoryCandidate}>
            ...
            {mutationError && <p className="os-palette-error" role="alert">{mutationError}</p>}
            <div className="os-palette-editor-actions"><button type="button" onClick={resetEditor}>Back</button><button className="is-primary" disabled={mutationPending || !memoryDraft.title.trim() || !memoryDraft.summary.trim()}>{mutationPending ? "Creating candidate…" : "Create candidate"}</button></div>
          </form>
        ) : (
          <>
            ...
            {statusMessage && <div className="os-palette-status" role="status"><span>{statusMessage}</span>{statusMessage.includes("candidate") && <button type="button" onClick={() => { navigate("/brain/inbox"); onClose(); }}>Open Memory Inbox</button>}</div>}
            ...
            <div id="command-palette-results" className="os-palette-results" role="listbox" aria-label="Command results">
              ...
              {grouped.map(({ group, commands }) => <div role="group" aria-labelledby={`palette-group-${group.replaceAll(" ", "-")}`} key={group} className="os-palette-group"><h3 id={`palette-group-${group.replaceAll(" ", "-")}`}>{group}</h3>{commands.map(({ command, index }) => (
                <button
                  type="button"
