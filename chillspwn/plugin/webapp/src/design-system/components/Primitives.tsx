...
export function ButtonLink({ href, variant = "primary", children, className = "" }: {
  ...
  return <AppLink href={href} className={`os-button os-button--${variant} ${className}`}>{children}</AppLink>;
}
...
export function LoadingPanel({ label = "Loading current system state" }: { label?: string }) {
  return (
    <div className="os-state-panel" role="status" aria-live="polite">
      ...
      <div><strong>{label}</strong><p>Waiting for the Command OS service to respond.</p></div>
    </div>
  );
}
