...
describe("Command OS operational response contracts", () => {
  test("parses immutable Autonomous branch lineage, versioned readiness, and the new-run result", () => {
    ...
    const preflight = parseAutonomousBranchPreflight({
      schemaVersion: "2.4", mode: "contract_amendment", sourceRunId: "run-1", sourceRunVersion: 4,
      ...
      request,
      preflight: {
        schemaVersion: "2.4", contract: { version: 2, hash: "b".repeat(64) },
    ...
    const result = parseAutonomousBranchResult({
      schemaVersion: "2.4", sourceRunId: "run-1", branchMode: "contract_amendment",
  ...
  test("parses scoped evidence pages and rejects incomplete records", () => {
    const parsed = parseEvidencePage({ schemaVersion: "2.4", nextCursor: "next", items: [{
    ...
    expect(() => parseEvidencePage({ schemaVersion: "2.4", nextCursor: null, items: [{ id: "ev-bad" }] })).toThrow();
  });

  test("validates provider metrics and review mutation envelopes", () => {
    expect(parseProviderPage({ schemaVersion: "2.4", nextCursor: null, items: [{ id: "p", provider: "grok", model: "expert", status: "operational", turnCount: 3, completedCount: 3, failedCount: 0, meanLatencyMs: 42, inputTokens: 10, outputTokens: 20, estimatedCost: null, lastTurnAt: null }] }).items[0].turnCount).toBe(3);
    expect(parseFindingReview({ schemaVersion: "2.4", finding: { id: "finding-1", reviewStatus: "verified", version: 2, evidenceCount: 1 } }).finding).toBeDefined();
  ...
  test("parses action-level and planning-level memory transparency projections", () => {
    const actions = parseActionPage({ schemaVersion: "2.4", nextCursor: null, items: [{
    ...
    const packs = parseMemoryContextPackPage({ schemaVersion: "2.4", totalReturned: 1, items: [{
    ...
    expect(() => parseActionPage({ schemaVersion: "2.4", nextCursor: null, items: [{ ...actions.items[0], status: "invented" }] })).toThrow("status");
  });
});

describe("Command runtime V2 response contracts", () => {
  test("parses mission, run, plan, and exact Guided decision projections", () => {
    const mission = parseMissionRuntime({ schemaVersion: "2.4", mission: { id: "mission-1", name: "Authorized mission", objective: "Verify target", journey: "guided", engagementId: "eng-1", authorizationStatus: "verified", allowedTargets: ["10.0.0.1"], prohibitedTargets: [], successCriteria: ["Evidence captured"], memoryPolicy: { allowed: true } }, runs: [run] });
    expect(mission.runs[0].status).toBe("waiting_guided_decision");
    const plans = parsePlans({ schemaVersion: "2.4", items: [{ id: "plan-1", runId: "run-1", version: 1, status: "active", strategySummary: "Bounded recon", rationaleSummary: "Establish services", createdAt: "2026-07-15T11:00:00.000Z", activatedAt: "2026-07-15T11:00:01.000Z", steps: [{ id: "step-1", ordinal: 0, phase: "Recon", title: "Map approved service", objective: "Find ports", status: "waiting", assignedAgentId: "recon", riskClass: "low", successCriteria: ["Unique service"], dependencyStepIds: [], explanation: "Map the approved host", rationale: "Reduce uncertainty", reversibility: "Read-only", action: { actionType: "scan", actionClass: "recon", target: "10.0.0.1", arguments: { ports: "80" }, intentSummary: "Map service", kind: "tool", idempotent: true, destructive: false } }] }] });
    expect(plans.items[0].steps[0].action.target).toBe("10.0.0.1");
    const decisions = parseDecisions({ schemaVersion: "2.4", items: [{ id: "decision-1", missionId: "mission-1", runId: "run-1", stepId: "step-1", status: "pending", actionFingerprint: "f".repeat(64), requestedParameters: { target: "10.0.0.1" }, rationale: "Map service", riskClass: "low", reversibility: "Read-only", expiresAt: "2026-07-15T13:00:00.000Z", createdAt: "2026-07-15T12:00:00.000Z" }] });
    expect(decisions.items[0].requestedParameters).toEqual({ target: "10.0.0.1" });
    expect(parseRunMutation({ schemaVersion: "2.4", run, latestCheckpoint: null }).run.status).toBe("waiting_guided_decision");
    expect(parseDecisionMutation({ schemaVersion: "2.4", decisionId: "decision-1", status: "cancelled", run: { ...run, status: "cancelled", endedAt: "2026-07-15T12:10:00.000Z" } }).run?.status).toBe("cancelled");
  });

  test("rejects hidden third journeys and unknown run states", () => {
    expect(() => parseMissionRuntime({ schemaVersion: "2.4", mission: { id: "m", name: "M", objective: "O", journey: "supervised", engagementId: null, authorizationStatus: "verified", allowedTargets: [], prohibitedTargets: [], successCriteria: [], memoryPolicy: {} }, runs: [] })).toThrow("journey");
    expect(() => parseRunMutation({ schemaVersion: "2.4", run: { ...run, status: "waiting_input" } })).toThrow("run status");
  });
});
