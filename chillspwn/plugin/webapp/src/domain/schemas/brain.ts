...
function schemaVersion(value: unknown): "2.1" {
  if (value !== "2.1") throw new Error("unsupported Second Brain schema version");
  return value;
}
...
export function parseVaultSnapshot(payload: unknown): VaultSnapshot {
  ...
  return {
    enabled: value.enabled === true, ...(optionalText(value.allowedRootLabel) ? { allowedRootLabel: optionalText(value.allowedRootLabel) } : {}),
