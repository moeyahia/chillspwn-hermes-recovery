...
function GuidedPortfolio() {
  ...
  return <div className="os-page"><PageHeader eyebrow="Collaborative mission control" title="Guided Workspace" description="Explain, recommend, choose, observe, interpret, record, and advance—one deliberate consequential step at a time." actions={<><StreamState /><ButtonLink href="/missions/new/guided">Start Guided Mission</ButtonLink></>} />
    ...
    <QueryBoundary data={runs} error={query.error} isLoading={query.isLoading} onRetry={query.refresh} emptyTitle="No Guided missions match" emptyDescription="Start a Guided mission or change the checkpoint filter.">{(items) => <div className="os-operation-list">{items.map((run) => <Card key={run.id}><div className="os-operation-main"><div><p className="os-eyebrow">{run.status === "waiting_guided_decision" ? "Waiting for your deliberate step" : run.currentOwnerId ?? "Commander"}</p><h2><AppLink href={`/guided/${encodeURIComponent(run.missionId)}`}>{run.missionName}</AppLink></h2><p>{run.objective}</p></div><StatusPill status={run.status} /></div><div className="os-operation-metrics"><span><small>Progress</small>{percent(run.progress)}</span><span><small>Checkpoint</small>{run.currentStepId ?? "Planning"}</span><span><small>Last activity</small>{formatTime(run.updatedAt)}</span><span><small>Next</small>{run.nextAction ?? "Awaiting plan"}</span></div></Card>)}</div>}</QueryBoundary>
  </div>;
}
  ...
  return <div className="os-page os-guided-mission-page"><PageHeader eyebrow="Guided mission" title={mission.data.mission.name} description={mission.data.mission.objective} actions={<><StatusPill status={run?.status ?? "not_started"} /><StreamState /></>} />
    ...
    {run && <div className="os-guided-workspace-layout">
      <aside className="os-guided-step-rail" aria-label="Current Guided step">
        {currentStep ? <Card className="os-guided-step"><p className="os-eyebrow">Explain → recommend → choose</p><h2>{currentStep.title}</h2><p>{currentStep.explanation || currentStep.objective}</p><div className="os-guided-step-detail"><div><h3>Why this matters</h3><p>{currentStep.rationale || "No rationale was returned by the versioned plan."}</p></div><div><h3>Expected evidence</h3><ul>{currentStep.successCriteria.map((criterion) => <li key={criterion}>{criterion}</li>)}</ul></div><div><h3>Risk and reversal</h3><p><StatusPill status={currentStep.riskClass || "unclassified"} /> {currentStep.reversibility || "Not reported"}</p></div></div><JsonDetails label="Exact proposed procedure" value={currentStep.action} /></Card> : <Card><LoadingPanel label={plans.isLoading ? "Loading the represented Guided step" : "Waiting for the runtime to represent the next Guided step"} /></Card>}
      </aside>
      <section className="os-guided-conversation-column" aria-label="Guided Commander conversation"><GuidedCommanderPanel missionId={missionId} runId={run.id} onAdvanced={() => { mission.refresh(); plans.refresh(); decisions.refresh(); }} /></section>
      <aside className="os-guided-decision-rail" aria-label="Exact Guided decision">
        ...
        {decisions.data?.items.map((decision) => <DecisionCard key={decision.id} decision={decision} onChanged={() => { decisions.refresh(); mission.refresh(); plans.refresh(); }} />)}
      </aside>
    </div>}
