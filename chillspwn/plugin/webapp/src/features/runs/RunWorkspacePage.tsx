...
function RunControls({ run, onChanged }: { run: RuntimeRun; onChanged: () => void }) {
  ...
  const command = (value: "pause" | "resume" | "cancel", event: FormEvent) => { event.preventDefault(); void action.run(() => runtimeV2Api.controlRun(run.id, value, reason, `run-${crypto.randomUUID()}`).then(onChanged), `Run ${value} recorded.`); };
  return <Card className="os-run-controls"><form onSubmit={(event) => command(run.status === "blocked" ? "resume" : "pause", event)}><label><span>Operator reason (audited)</span><input required value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Why is this intervention necessary?" /></label><div><Button variant="secondary" disabled={terminal || action.pending || !reason.trim()}>{run.status === "blocked" ? "Resume from checkpoint" : "Pause run"}</Button><Button type="button" variant="danger" disabled={terminal || action.pending || !reason.trim()} onClick={(event) => command("cancel", event as unknown as FormEvent)}>Cancel run</Button></div></form>{action.error && <ErrorPanel error={action.error} />}{action.message && <p role="status" className="os-success-note">{action.message}</p>}</Card>;
}
