...
export default function BrainHomePage() {
  ...
  const [status, setStatus] = useState<MemoryLifecycle | "">("");
  const [sensitivity, setSensitivity] = useState<MemorySensitivity | "">("");
  const filters = useMemo(() => ({ query: submitted || undefined, nodeType: nodeType || undefined, status: status || undefined, sensitivity: sensitivity || undefined, limit: 50 }), [submitted, nodeType, status, sensitivity]);
  ...
  return (
    <div className="os-page brain-page">
      ...
      <Card className="brain-browser">
        ...
        <form className="brain-search" onSubmit={(event) => { event.preventDefault(); setSubmitted(query.trim()); }} role="search">
          ...
          <label>Node type<select value={nodeType} onChange={(event) => setNodeType(event.target.value as MemoryNodeType | "")}><option value="">All types</option>{MEMORY_NODE_TYPES.map((type) => <option key={type} value={type}>{type.replaceAll("_", " ")}</option>)}</select></label>
          <label>Lifecycle<select value={status} onChange={(event) => setStatus(event.target.value as MemoryLifecycle | "")}><option value="">All active states</option>{MEMORY_LIFECYCLE_STATES.filter((item) => item !== "forgotten").map((item) => <option key={item}>{item}</option>)}</select></label>
          <label>Sensitivity<select value={sensitivity} onChange={(event) => setSensitivity(event.target.value as MemorySensitivity | "")}><option value="">Permitted levels</option>{MEMORY_SENSITIVITIES.map((item) => <option key={item}>{item}</option>)}</select></label>
        ...
        {nodes.data && nodes.data.items.length > 0 && (
          <div className="os-table-scroll"><table><thead><tr><th>Memory</th><th>Type</th><th>State</th><th>Scope</th><th>Confidence</th><th>Sources</th><th>Updated</th></tr></thead><tbody>{nodes.data.items.map((node) => <tr key={node.id}><th scope="row"><ButtonLink href={`/brain/nodes/${encodeURIComponent(node.id)}`} variant="quiet">{node.title}</ButtonLink><small className="brain-table-summary">{node.summary}</small></th><td>{node.nodeType.replaceAll("_", " ")}</td><td><StatusPill status={node.lifecycleStatus} /></td><td>{scopeLabel(node.scope)}</td><td>{Math.round(node.confidence * 100)}%</td><td>{node.sourceCount}</td><td>{formatBrainDate(node.updatedAt)}</td></tr>)}</tbody></table></div>
        )}
        {nodes.data?.nextCursor && <p className="brain-cursor-note">More results are available. Narrow the query or open the graph to expand a bounded neighborhood.</p>}
      </Card>
    </div>
  );
}
