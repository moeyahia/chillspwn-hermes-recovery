    ...
    <div className="os-page brain-page brain-vault-page">
      ...
      {snapshot && <>
        ...
        <Card className="brain-vault-connect">
          ...
          <div className="os-field-grid"><label>Display name<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>Path inside the allowed root<input value={path} onChange={(event) => { setPath(event.target.value); setHealth(undefined); }} placeholder="ChillsPwn-Brain" /></label></div>
          <label className="os-check-field"><input type="checkbox" aria-label="Grant explicit filesystem permission" aria-describedby="brain-vault-filesystem-permission-help" checked={permission} onChange={(event) => { setPermission(event.target.checked); if (!event.target.checked) setHealth(undefined); }} /><span><strong>Grant explicit filesystem permission</strong><small id="brain-vault-filesystem-permission-help">Allow ChillsPwn to create and atomically synchronize Markdown notes inside this selected vault only.</small></span></label>
