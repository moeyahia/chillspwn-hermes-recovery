...
export default function BrainGraphPage() {
  ...
  const applySavedView = (saved: SavedBrainGraphView) => {
    ...
    if (saved.state.view === "local" && saved.state.rootNodeId) sessionStorage.setItem("chillspwn.brain.graph-root", saved.state.rootNodeId);
    else sessionStorage.removeItem("chillspwn.brain.graph-root");
