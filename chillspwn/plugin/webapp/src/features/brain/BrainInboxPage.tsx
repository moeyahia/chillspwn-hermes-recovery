...
function CandidateCard({ candidate, onChanged }: { candidate: MemoryCandidate; onChanged: () => void }) {
  ...
  const reject = async () => {
    ...
    try {
      ...
      setState({ busy: false, message: result.suppressionId ? "Rejected and suppressed from immediate relearning." : "Memory candidate rejected." });
      onChanged();
  ...
  return (
    <article className="brain-candidate">
      ...
      {state.error && <p className="brain-mutation-note is-error" role="alert">{state.error.message}</p>}
      <footer><Button onClick={confirm} disabled={state.busy || !scopeValid || !title.trim() || !summary.trim()}>{editing ? "Confirm edited memory" : "Confirm memory"}</Button><Button variant="secondary" onClick={() => setEditing((value) => !value)} disabled={state.busy}>{editing ? "Discard edits" : "Edit then confirm"}</Button><Button variant="quiet" onClick={() => setRejecting((value) => !value)} disabled={state.busy}>{rejecting ? "Cancel rejection" : "Reject"}</Button></footer>
    </article>
  );
}
...
export default function BrainInboxPage() {
  ...
  return (
    <div className="os-page brain-page">
      ...
      {candidates.data?.nextCursor && <p className="brain-cursor-note">Additional candidates are available from the service. Complete this review batch before loading the next bounded page.</p>}
    </div>
  );
}
