...
import { ButtonLink, Card, EmptyState, PageHeader, StatusPill } from "../../design-system/components/Primitives";
...
function LivePortfolio() {
  ...
  return <div className="os-page"><PageHeader eyebrow="Autonomous observation" title="Live Operations" description="Durable Autonomous execution, recovery, heartbeat, ownership, and next action—without a chat dependency." actions={<><StreamState /><ButtonLink href="/missions/new/autonomous">Go Autonomous</ButtonLink></>} />
    ...
    <QueryBoundary data={runs} error={query.error} isLoading={query.isLoading} onRetry={query.refresh} emptyTitle="No Autonomous runs match" emptyDescription="Launch an authorized Autonomous mission or adjust the state filter.">{(items) => <div className="os-operation-list">{items.map((run) => <Card key={run.id}><div className="os-operation-main"><div><p className="os-eyebrow">{run.currentOwnerId ?? "Supervisor"}</p><h2><AppLink href={`/live/${encodeURIComponent(run.id)}`}>{run.missionName}</AppLink></h2><p>{run.objective}</p></div><StatusPill status={run.status}>{run.status === "running" ? "Executing autonomously" : run.status === "recovering" ? "Recovering autonomously" : run.status.replaceAll("_", " ")}</StatusPill></div><div className="os-operation-metrics"><span><small>Phase/step</small>{run.currentStepId ?? "Planning"}</span><span><small>Progress</small>{percent(run.progress)}</span><span><small>Heartbeat</small>{formatTime(run.lastHeartbeatAt)}</span><span><small>Next action</small>{run.nextAction ?? "Not reported"}</span></div><div className="os-progress-track" aria-label={`${percent(run.progress)} complete`}><span style={{ width: percent(run.progress) }} /></div></Card>)}</div>}</QueryBoundary>
