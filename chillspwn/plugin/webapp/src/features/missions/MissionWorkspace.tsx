...
function HistoryPanel({ missionId }: { missionId: string }) {
  ...
  return <><QueryBoundary data={query.data?.items} error={query.error} isLoading={query.isLoading} onRetry={query.refresh} emptyTitle="No mission history" emptyDescription="No append-only event has been recorded for this mission.">{(items) => <ol className="os-semantic-feed">{items.map((event) => <li key={event.id}><div className="os-feed-marker" /><article><header><div><strong>{event.summary}</strong><span>{event.eventType} · {event.actor.id ?? event.actor.type}</span></div><time>{formatTime(event.occurredAt)}</time></header><p className="os-feed-meta">Run {event.runId ?? "mission-level"} · sequence {event.sequence ?? "—"}</p>{event.correlation.contextPackId && <ContextUsedDisclosure packId={event.correlation.contextPackId} />}<JsonDetails label="Technical event detail" value={{ payload: event.payload, correlation: event.correlation, redaction: event.redaction }} /></article></li>)}</ol>}</QueryBoundary><ActionActivity missionId={missionId} title="Action history" /></>;
}
