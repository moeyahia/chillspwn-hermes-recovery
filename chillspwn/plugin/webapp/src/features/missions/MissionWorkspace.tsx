...
function MissionTabContent({ activeTab, snapshot, run, liveContent }: {
  ...
  if (activeTab === "plan") return <PlanPanel run={run} />;
  ...
  if (activeTab === "evidence") return <EvidencePanel missionId={snapshot.mission.id} />;
  ...
  return <div className="os-mission-summary">
    ...
    {run && <>
      ...
      <div className="os-mission-summary-grid">
        <Card><p className="os-eyebrow">Current work</p><h2>{currentStep?.title ?? run.nextAction ?? "No active step reported"}</h2><p>{currentStep?.explanation || currentStep?.objective || run.statusReason || "The runtime has not reported a current-step explanation."}</p><KeyValueGrid items={[{ label: "Owner", value: run.currentOwnerId ?? "Supervisor" }, { label: "Checkpoint", value: run.currentStepId ?? "Not started" }, { label: "Progress", value: percent(run.progress) }, { label: "Next action", value: run.nextAction ?? "Not reported" }]} /></Card>
    ...
    {[plans.error, evidence.error, findings.error, decisions.error].some(Boolean) && <DegradedNotice>One or more summary projections could not refresh. Visible records remain the last validated canonical state.</DegradedNotice>}
  </div>;
}

function PlanPanel({ run }: { run?: RuntimeRun }) {
  const plans = useQuery(`run-plans:${run?.id ?? "none"}`, (signal) => run ? runtimeV2Api.plans(run.id, signal) : Promise.resolve(undefined), { staleTime: 0 });
  ...
  const activePlan = plans.data?.items.find((plan) => plan.id === run.currentPlanId) ?? plans.data?.items[0];
  if (!activePlan) return <Card><EmptyState title="No plan available" description="The runtime has not persisted a versioned plan for this run." /></Card>;
  return <PlanView plan={activePlan} currentStepId={run.currentStepId} />;
}

function PlanView({ plan, currentStepId }: { plan: RunPlan; currentStepId: string | null }) {
  return <section aria-labelledby={`mission-plan-${plan.id}`}><Card className="os-plan-summary"><div className="os-card-heading"><div><p className="os-eyebrow">Plan v{plan.version}</p><h2 id={`mission-plan-${plan.id}`}>{plan.strategySummary}</h2></div><StatusPill status={plan.status} /></div>{plan.rationaleSummary && <p>{plan.rationaleSummary}</p>}<KeyValueGrid items={[{ label: "Created", value: formatTime(plan.createdAt) }, { label: "Activated", value: formatTime(plan.activatedAt) }, { label: "Steps", value: plan.steps.length }]} /></Card>
    <ol className="os-step-rail">{plan.steps.map((step) => <li key={step.id} className={step.id === currentStepId ? "is-current" : undefined}><div className="os-step-index">{step.ordinal + 1}</div><article><header><div><span>{step.phase}</span><h3>{step.title}</h3></div><StatusPill status={step.status} /></header><p>{step.explanation || step.objective}</p><KeyValueGrid items={[{ label: "Owner", value: step.assignedAgentId || "Unassigned" }, { label: "Risk", value: step.riskClass || "Not classified" }, { label: "Target", value: step.action.target || "Not reported" }, { label: "Reversibility", value: step.reversibility || "Not reported" }]} /><p className="os-intent"><strong>Intent:</strong> {step.action.intentSummary}</p><JsonDetails label="Normalized action detail" value={step.action} /></article></li>)}</ol>
  </section>;
}
...
function GuidePanel({ missionId, run }: { missionId: string; run?: RuntimeRun }) {
  ...
  return <div className="os-mission-guide-grid">
    <section>{plans.isLoading && <LoadingPanel label="Loading represented Guided step" />}{plans.error && !plans.data && <ErrorPanel error={plans.error} onRetry={plans.refresh} />}{step ? <Card className="os-guided-step"><p className="os-eyebrow">Explain → recommend → choose</p><h2>{step.title}</h2><p>{step.explanation || step.objective}</p><div className="os-guided-columns"><div><h3>Why this matters</h3><p>{step.rationale || "No rationale was returned by the versioned plan."}</p></div><div><h3>Expected evidence</h3>{step.successCriteria.length ? <ul>{step.successCriteria.map((criterion) => <li key={criterion}>{criterion}</li>)}</ul> : <p>Not specified</p>}</div><div><h3>Risk and reversal</h3><p><StatusPill status={step.riskClass || "unclassified"} /> {step.reversibility || "Not reported"}</p></div></div><JsonDetails label="Exact proposed procedure" value={step.action} /></Card> : !plans.isLoading && <Card><EmptyState title="No represented Guided step" description="The runtime has not persisted a current deliberate step." /></Card>}</section>
    <aside><p className="os-eyebrow">Exact-step boundary</p><h2>Guided decision</h2><p>Only the fingerprint-bound decision below may execute or record this represented action.</p>{decisions.isLoading && <LoadingPanel label="Loading exact Guided decision" />}{decisions.error && !decisions.data && <ErrorPanel error={decisions.error} onRetry={decisions.refresh} />}{decisions.data?.items.map((decision) => <DecisionCard key={decision.id} decision={decision} onChanged={() => decisions.refresh()} />)}{decisions.data?.items.length === 0 && <Card><EmptyState title="No decision is waiting" description="This Guided checkpoint has no pending consequential action." /></Card>}<ButtonLink href={`/guided/${encodeURIComponent(missionId)}`} variant="secondary">Open focused Guided Workspace</ButtonLink></aside>
  </div>;
}

function EvidencePanel({ missionId }: { missionId: string }) {
  const query = useQuery(`evidence:mission:${missionId}`, (signal) => operationsApi.evidence({ missionId, limit: 100 }, signal), { staleTime: 0 });
  return <QueryBoundary data={query.data?.items} error={query.error} isLoading={query.isLoading} onRetry={query.refresh} emptyTitle="No evidence retained" emptyDescription="Authorized actions have not yet produced a hashed evidence record for this mission.">{(items) => <div className="os-table-wrap"><table className="os-data-table"><thead><tr><th>Evidence</th><th>Type</th><th>Target</th><th>Verification</th><th>Acquired</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><th scope="row"><AppLink href={`/intelligence/evidence/${encodeURIComponent(item.id)}`}>{item.summary || item.id}</AppLink><small className="os-mono">{item.contentHash.slice(0, 12)}…</small></th><td>{item.evidenceType}</td><td>{item.target ?? "Not reported"}</td><td><StatusPill status={item.verificationState} /></td><td>{formatTime(item.acquiredAt)}</td></tr>)}</tbody></table></div>}</QueryBoundary>;
}
...
function ConversationPanel({ missionId, run }: { missionId: string; run?: RuntimeRun }) {
  if (!run) return <NoRunState />;
  if (run.journey === "guided") return <GuidedCommanderPanel missionId={missionId} runId={run.id} />;
...
function BrainPanel({ missionId, run }: { missionId: string; run?: RuntimeRun }) {
  ...
  const contextPackIds = useMemo(() => [...new Set(events.data?.items.map((event) => event.correlation.contextPackId).filter((id): id is string => Boolean(id)) ?? [])], [events.data?.items]);
  const [selectedPack, setSelectedPack] = useState<string>();
  return <div className="os-mission-brain-grid"><section><div className="os-card-heading"><div><p className="os-eyebrow">Mission-isolated memory</p><h2>Second Brain nodes</h2></div><ButtonLink href="/brain/graph" variant="secondary">Open graph</ButtonLink></div><QueryBoundary data={nodes.data?.items} error={nodes.error} isLoading={nodes.isLoading} onRetry={nodes.refresh} emptyTitle="No mission memory nodes" emptyDescription="No confirmed or verified memory has been scoped to this mission yet.">{(items) => <ul className="os-compact-list">{items.map((node) => <li key={node.id}><span><AppLink href={`/brain/nodes/${encodeURIComponent(node.id)}`}>{node.title}</AppLink><small>{node.nodeType.replaceAll("_", " ")} · {scopeLabel(node.scope)} · {Math.round(node.confidence * 100)}% confidence</small></span><StatusPill status={node.lifecycleStatus} /></li>)}</ul>}</QueryBoundary></section><aside><Card><p className="os-eyebrow">Why the system knew something</p><h2>Context packs used</h2><p>These IDs come from durable mission events. Open one to inspect retrieved nodes and their recorded influence.</p>{contextPackIds.length ? <div className="os-context-pack-links">{contextPackIds.map((packId) => <Button key={packId} variant="quiet" onClick={() => setSelectedPack(selectedPack === packId ? undefined : packId)}>{selectedPack === packId ? "Hide context" : packId}</Button>)}</div> : <p className="os-muted">No event has recorded a context pack for this mission.</p>}{selectedPack && <ContextPackPanel packId={selectedPack} />}{run && <p className="os-muted">Selected run: <span className="os-mono">{run.id}</span></p>}</Card></aside></div>;
}
...
function HistoryPanel({ missionId }: { missionId: string }) {
  ...
  return <QueryBoundary data={query.data?.items} error={query.error} isLoading={query.isLoading} onRetry={query.refresh} emptyTitle="No mission history" emptyDescription="No append-only event has been recorded for this mission.">{(items) => <ol className="os-semantic-feed">{items.map((event) => <li key={event.id}><div className="os-feed-marker" /><article><header><div><strong>{event.summary}</strong><span>{event.eventType} · {event.actor.id ?? event.actor.type}</span></div><time>{formatTime(event.occurredAt)}</time></header><p className="os-feed-meta">Run {event.runId ?? "mission-level"} · sequence {event.sequence ?? "—"}</p>{event.correlation.contextPackId && <ContextUsedDisclosure packId={event.correlation.contextPackId} />}<JsonDetails label="Technical event detail" value={{ payload: event.payload, correlation: event.correlation, redaction: event.redaction }} /></article></li>)}</ol>}</QueryBoundary>;
}

function SettingsPanel({ snapshot, selectedRun }: { snapshot: MissionRuntimeSnapshot; selectedRun?: RuntimeRun }) {
  return <div className="os-mission-settings-grid"><Card><p className="os-eyebrow">Authorization</p><h2>Mission settings</h2><p>These values are the current canonical mission contract projection. Changes require a versioned mission or contract amendment; this workspace does not silently mutate scope.</p><KeyValueGrid items={[{ label: "Journey", value: snapshot.mission.journey === "autonomous" ? "Autonomous" : "Guided" }, { label: "Authorization", value: snapshot.mission.authorizationStatus }, { label: "Engagement", value: snapshot.mission.engagementId ?? "Not assigned" }, { label: "Mission ID", value: <span className="os-mono">{snapshot.mission.id}</span> }]} /><JsonDetails label="Memory policy" value={snapshot.mission.memoryPolicy} /></Card><Card><p className="os-eyebrow">Scope boundaries</p><h2>Targets and criteria</h2><h3>Allowed targets</h3>{snapshot.mission.allowedTargets.length ? <ul>{snapshot.mission.allowedTargets.map((target) => <li key={target}>{target}</li>)}</ul> : <p className="os-muted">None recorded.</p>}<h3>Prohibited targets</h3>{snapshot.mission.prohibitedTargets.length ? <ul>{snapshot.mission.prohibitedTargets.map((target) => <li key={target}>{target}</li>)}</ul> : <p className="os-muted">None recorded.</p>}<h3>Success criteria</h3>{snapshot.mission.successCriteria.length ? <ul>{snapshot.mission.successCriteria.map((criterion) => <li key={criterion}>{criterion}</li>)}</ul> : <p className="os-muted">None recorded.</p>}</Card><Card><p className="os-eyebrow">Durable attempts</p><h2>Mission runs</h2>{snapshot.runs.length ? <ul className="os-compact-list">{snapshot.runs.map((run) => <li key={run.id}><span><AppLink href={`/missions/${encodeURIComponent(snapshot.mission.id)}/runs/${encodeURIComponent(run.id)}`}>{run.id}</AppLink><small>Updated {formatTime(run.updatedAt)} · {percent(run.progress)}</small></span><StatusPill status={run.id === selectedRun?.id ? "selected" : run.status}>{run.id === selectedRun?.id ? `Selected · ${run.status}` : run.status}</StatusPill></li>)}</ul> : <p className="os-muted">No execution attempts are attached.</p>}</Card></div>;
}

function EventList({ items }: { items: readonly EventRecord[] }) {
  return <ol className="os-timeline">{items.map((event) => <li key={event.id}><strong>{event.summary}</strong><span>{event.eventType} · {formatTime(event.occurredAt)}</span>{event.correlation.contextPackId && <ContextUsedDisclosure packId={event.correlation.contextPackId} />}<JsonDetails label="Technical event detail" value={{ payload: event.payload, correlation: event.correlation }} /></li>)}</ol>;
}
