...
const LEGACY_SAVED_VIEWS_KEY = "chillspwn.missions.saved-views.v1";
  ...
  return (
    <article className="mission-board-card">
      <div className="os-card-heading">
        <div><p className="os-eyebrow">{mission.journey}</p><h3><AppLink href={missionHref(mission)}>{mission.title}</AppLink></h3></div>
        <StatusPill status={mission.status} />
      </div>
      <label className="mission-select"><input type="checkbox" aria-label={`Select ${mission.title}`} checked={selected} onChange={(event) => onSelected(event.target.checked)} /><span>Select mission</span></label>
      <dl>
        ...
        <div><dt>Next</dt><dd>{mission.nextAction ?? "No next action reported"}</dd></div>
      </dl>
...
export default function MissionPortfolioPage() {
  ...
  const missions = useQuery(`command-os-missions:${filters.key}`, (signal) => fetchMissions({
  ...
  const saved = useQuery("command-os-mission-saved-views", fetchSavedMissionViews, { staleTime: 5_000 });
  ...
  useEffect(() => {
    if (!saved.data || legacyImportStarted.current) return;
    const legacyRaw = localStorage.getItem(LEGACY_SAVED_VIEWS_KEY);
    const legacy = parseSavedMissionViews(legacyRaw);
    if (!legacyRaw) return;
    legacyImportStarted.current = true;
    void (async () => {
      try {
        let collection = saved.data!;
        for (const view of legacy) {
          collection = await saveMissionView(
            { expectedVersion: collection.version, name: view.name, state: view.state },
            `legacy-view-import-${view.id}`.slice(0, 200),
          );
        }
        localStorage.removeItem(LEGACY_SAVED_VIEWS_KEY);
        saved.refresh();
      } catch (cause) {
        setViewError(cause instanceof Error ? cause : new Error("Legacy view import failed"));
      }
    })();
  }, [saved.data]);
  ...
  const confirmBulk = async () => {
    ...
    try {
      ...
      if (bulkMode === "archive") {
        ...
        link.href = url;
        link.download = `chillspwn-mission-metadata-${result.selectionHash.slice(0, 12)}.json`;
        link.click();
  ...
  return (
    <div className="os-page">
      ...
      <section className="mission-saved-views" aria-labelledby="mission-saved-title">
        <div><strong id="mission-saved-title">Synchronized saved views</strong><span>Actor-scoped filter and layout settings are versioned on the Command OS server. Mission and evidence content is never copied into a view.</span></div>
          ...
          <div className="os-table-scroll">
            <table>
              ...
              <tbody>{missions.data.items.map((mission) => (
                <tr key={mission.id}>
                  <td><label className="mission-row-select"><input type="checkbox" aria-label={`Select ${mission.title}`} checked={selected.has(mission.id)} onChange={(event) => setMissionSelected(mission.id, event.target.checked)} /></label></td>
                  <th scope="row"><ButtonLink variant="quiet" href={missionHref(mission)}>{mission.title}</ButtonLink><span>{mission.engagementId ?? "No engagement"}</span><small>{scopeSummary(mission)}{mission.scope.prohibitedTargetCount > 0 ? ` · ${mission.scope.prohibitedTargetCount} prohibited` : ""}</small></th>
                  ...
                  <td>{mission.lastMeaningfulEvent ? <><strong>{mission.lastMeaningfulEvent.summary}</strong><time dateTime={mission.lastMeaningfulEvent.occurredAt}>{dateTime(mission.lastMeaningfulEvent.occurredAt)}</time></> : "No semantic event reported"}</td>
                  ...
                  <td>{mission.nextAction ?? "Not reported"}{mission.progress !== null && <small>{Math.round(mission.progress)}% complete</small>}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
