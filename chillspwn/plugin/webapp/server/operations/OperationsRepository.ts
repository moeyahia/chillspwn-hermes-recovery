...
export class OperationsRepository {
  ...
  listFindings(
    ...
    const rows = this.database.prepare(`
      SELECT f.*, f.updated_at AS sort_at, m.name AS mission_name,
        ...
        SUM(CASE WHEN e.verification_state = 'verified' THEN 1 ELSE 0 END) AS verified_evidence_count
  ...
  getArtifact(id: string, access: OperationsAccessPolicy): Record<string, unknown> {
    ...
    return { schemaVersion: OPERATIONS_SCHEMA_VERSION, ...this.mapArtifact(row) };
  }
   ...
   */
  getRunCompletionExport(runId: string, access: OperationsAccessPolicy): RunCompletionExport {
    const scope = missionScopeSql("m", access);
    const row = this.database.prepare(`
