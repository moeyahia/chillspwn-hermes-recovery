...
function redactString(value: string): string {
  return value
    ...
    .replace(
      /\b(api[_-]?key|authorization|client[_-]?secret|credential|password|passwd|private[_-]?key|refresh[_-]?token|secret|session[_-]?token|token)\s*[:=]\s*["']?[^\s,;"'&]{4,}/giu,
      "$1=[REDACTED]",
    )
...
export function sanitizeJson(value: unknown, depth = 0): unknown {
  ...
  if (value && typeof value === "object") {
    ...
    for (const [key, child] of Object.entries(value as Record<string, unknown>).slice(0, 1_000)) {
      safe[key] = SENSITIVE_KEY.test(key) ? "[REDACTED]" : sanitizeJson(child, depth + 1);
    }
    return safe;
  }
  return value;
