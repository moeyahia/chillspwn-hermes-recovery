...
interface DownloadArtifactRow {
  ...
  readonly journey: "autonomous" | "guided";
  readonly mission_control_plane: "legacy" | "command_os_v2";
  readonly linked_run_id: string | null;
  readonly run_control_plane: "legacy" | "command_os_v2" | null;
...
export class SecureExportService {
  ...
  #describeArtifact(
    ...
    if (
      artifact.mission_control_plane !== "command_os_v2"
      || (artifact.run_id !== null && (
        artifact.linked_run_id !== artifact.run_id
        || artifact.run_control_plane !== "command_os_v2"
      ))
    ) {
      return deliveryDescriptor({
        state: "reconciliation_required",
        code: "artifact_control_plane_reconciliation_required",
        reason: "Artifact content is not owned by one canonical Command OS V2 mission and run boundary.",
