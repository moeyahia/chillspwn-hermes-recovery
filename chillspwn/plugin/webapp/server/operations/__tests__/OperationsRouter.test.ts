...
describe("canonical operations HTTP API", () => {
    ...
    try {
      ...
      expect(await body(rejected)).toMatchObject({
        error: {
          ...
          humanMessage: "This run belongs to the legacy control plane and Command OS V2 refused to mutate it.",
      ...
      database.prepare(`UPDATE runs SET control_plane = 'command_os_v2' WHERE id = 'run-control-plane-recovery'`).run();
      ...
      expect((database.prepare(`SELECT count(*) AS count FROM settings WHERE key LIKE 'command_os.recovery.idempotency.%'`).get() as { count: number }).count).toBe(0);
  ...
  test("preserves a pre-plan rate-limit safe stop and requires a new run", async () => {
    const { database, url } = await application();
    try {
      database.prepare(`
      ...
      expect(recovery.proposedRecovery.summary).toContain("start a new run");
      expect(recovery.actions.find((item: any) => item.kind === "resume").reason)
