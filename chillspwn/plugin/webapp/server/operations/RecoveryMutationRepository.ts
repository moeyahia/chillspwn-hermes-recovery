...
function idempotencySettingKey(key: string): string {
  return settingDigest("command_os.recovery.idempotency", key);
}
...
export class RecoveryMutationRepository {
  ...
  requestReplan(
    ...
    return inImmediateTransaction(this.database, () => {
      const replay = this.idempotentReplay(idempotencyKey, requestHash);
      ...
      this.appendAudit({
        ...
        details: { eventId: event.id, checkpointId: checkpoint.id, continuationId: continuation.id, strategyFingerprint: fingerprint },
        now,
      });
  ...
  reassignSpecialist(
    ...
    return inImmediateTransaction(this.database, () => {
      ...
      if (
        ...
        /commander/iu.test(candidate.role) || /chillspwn/iu.test(candidate.id)
      ) {
      ...
      this.assertJourneyPolicy(row, "reassign", input);
      const now = this.clock().toISOString();
      const guidedDecision = row.journey === "guided"
        ? this.database.prepare(`
            SELECT id, expires_at FROM guided_decisions
            WHERE id = ? AND run_id = ? AND step_id = ? AND status = 'pending'
          `).get(input.guidedDecisionId, runId, row.step_id) as {
            id: string;
            expires_at: string;
          } | undefined
        : undefined;
      ...
      if (guidedDecision) {
        ...
        const remainingTtlMs = Math.max(1_000, Date.parse(guidedDecision.expires_at) - Date.parse(now));
    ...
    return inImmediateTransaction(this.database, () => {
      ...
      if (!this.providerRouteIds.has(input.providerId)) {
        throw new OperationsApiError(409, "provider_route_not_callable", "The requested provider has no callable runtime route", {
          humanMessage: "The requested provider is not connected to the Command OS execution adapter.",
  ...
  private context(runId: string, access: OperationsAccessPolicy): RecoveryContextRow {
    ...
    const row = this.database.prepare(`
      SELECT r.id AS run_id, r.mission_id, r.journey, r.status AS run_status,
  ...
  private assertJourneyPolicy(
    ...
    if (operation === "replan") return;
    const decision = this.database.prepare(`
      SELECT id, requested_action_fingerprint, status, step_id
      FROM guided_decisions WHERE id = ? AND run_id = ?
    `).get(input?.guidedDecisionId, row.run_id) as {
      id: string; requested_action_fingerprint: string; status: string; step_id: string;
    } | undefined;
    if (
  ...
  private idempotentReplay(key: string, requestHash: string): RecoveryMutationProjection | null {
    const row = this.database.prepare("SELECT value_json FROM settings WHERE key = ?")
