...
export class ScriptArtifactService {
  ...
  private assertMission(missionId: string, mutable: boolean): MissionRow {
    ...
    if (mutable && row.control_plane !== "command_os_v2") {
      throw conflict("script_control_plane_conflict", "Legacy-controlled missions are read-only in Command OS V2", "Import or explicitly transfer the mission before creating V2 script versions.");
    }
    return row;
  }
  ...
  private assertRun(missionId: string, runId: string, mutable: boolean): RunRow {
    ...
    if (mutable && row.control_plane !== "command_os_v2") {
      throw conflict("script_control_plane_conflict", "Legacy-controlled runs are read-only in Command OS V2", "Transfer run ownership before creating V2 script versions.");
    }
    return row;
  }
