...
describe("CommandOsApplication", () => {
  test("reopens a valid canonical store and fails closed when the existing image is corrupt", async () => {
    const directory = mkdtempSync(join(tmpdir(), "command-os-app-integrity-"));
    temporaryDirectories.push(directory);
    const databasePath = join(directory, "command-os.sqlite");
    const snapshot = {
    ...
    writeFileSync(databasePath, "corrupt-existing-command-os-store", { flag: "w" });
  ...
  test("migrates, starts, serves real health, and stops cleanly", async () => {
    ...
    const operationalTruthLeaseOwner = "command-os-application-truth-runtime";
    const operationalTruthLeaseToken = "command-os-application-truth-token-000000";
    ...
    const controlLeaseOwner = "command-os-application-test-runtime";
    const controlLeaseToken = "command-os-application-test-token-000000";
