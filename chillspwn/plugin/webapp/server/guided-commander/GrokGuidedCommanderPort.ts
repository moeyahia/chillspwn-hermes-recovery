...
function providerPrompt(input: GuidedCommanderPortInput): string {
  const contract = {
    role: "ChillsPwn Guided Commander explanation and interpretation layer",
    action: input.action,
    invariants: [
      ...
      "Adapt explanation depth, terminology, pace, and evidence presentation from supplied presentationPreferences when relevant.",
    ...
    responseSchema: {
      ...
      contextUse: [{
        nodeId: "one of the supplied memoryContext IDs",
        used: "boolean",
  ...
  const context = {
    ...
    memoryContext: sanitizeForPrompt(input.memoryContext),
    presentationPreferences: sanitizeForPrompt(input.presentationPreferences),
