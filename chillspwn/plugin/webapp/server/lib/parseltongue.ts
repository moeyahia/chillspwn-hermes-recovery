...
// ── Safe Leetspeak (ASCII-only) ──────────────────────────────────────
// Uses ONLY ASCII characters — critical for LLM readability.
// Unicode homoglyphs cause Claude to misparse the prompt entirely.

const ASCII_LEET_MAP: Record<string, string[]> = {
  'a': ['4', '@'],
  'b': ['8', '|3'],
  'c': ['(', '<'],
  'd': ['|)', '|>'],
...
function applyAsciiLeet(word: string, intensity: 'light' | 'medium' | 'heavy'): string {
  const chars = word.split('')
  const transformCount = intensity === 'light' ? 1 : intensity === 'medium' ? Math.ceil(chars.length / 2) : chars.length
  const indices: number[] = []
  const step = Math.max(1, Math.floor(chars.length / transformCount))
  for (let i = 0; i < chars.length && indices.length < transformCount; i += step) {
    if (ASCII_LEET_MAP[chars[i].toLowerCase()]) indices.push(i)
  }
  if (indices.length < transformCount) {
    for (let i = 0; i < chars.length && indices.length < transformCount; i++) {
      if (ASCII_LEET_MAP[chars[i].toLowerCase()] && !indices.includes(i)) indices.push(i)
    }
  }
  const result = [...chars]
  for (const idx of indices) {
    const lower = result[idx].toLowerCase()
    const opts = ASCII_LEET_MAP[lower]
    if (opts) {
      const replacement = opts[Math.floor(Math.random() * opts.length)]
      result[idx] = result[idx] === lower ? replacement : replacement.toUpperCase()
    }
  }
  return result.join('')
}
...
function obfuscateWord(
  ...
  switch (technique) {
    case 'leetspeak':
      return applyAsciiLeet(word, intensity)
    case 'unicode':
