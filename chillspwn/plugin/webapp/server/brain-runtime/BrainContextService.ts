...
export class BrainContextService {
  ...
  #buildProviderContext(result: ProviderContextSource): BuiltProviderContext {
    ...
    for (const item of result.items) {
      if (
        item.node.nodeType === "lesson" &&
        !verifiedPlanningLesson(this.#database, result.contextPack, item.node, new Date().toISOString())
      ) {
        rejected.push({ nodeId: item.node.id, reason: "lesson_not_canonically_verified" });
        continue;
      }
  ...
  #validateCanonicalScope(request: BrainContextRequest): CanonicalScope {
    ...
    if (mission.control_plane !== "command_os_v2") {
