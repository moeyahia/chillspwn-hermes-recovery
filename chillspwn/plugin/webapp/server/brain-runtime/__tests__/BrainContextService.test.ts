  ...
  test("fails a required Autonomous policy hook and sanitizes only independently disclosure-approved memory", () => {
    ...
    try {
      ...
      db.prepare(`
        ...
        VALUES ('provider-turn-sanitized-context', ?, 'test-provider', 'test-model', 'started', ?)
      `).run(scope.runId, "2026-07-16T12:00:00.000Z");
      const receipted = runtime.prepareProviderContext(result, {
        providerTurnId: "provider-turn-sanitized-context",
        providerId: "test-provider",
        modelId: "test-model",
      });
      expect(receipted.exposureReceiptId).toMatch(/^exposure_/);
      expect(JSON.stringify(receipted)).not.toContain("body must never cross");
      expect(db.prepare(`
        SELECT provider_turn_id, selected_context_ids_json, rejected_context_ids_json,
          exposed_payload_hash, blocked
        FROM provider_exposure_receipts WHERE id = ?
      `).get(receipted.exposureReceiptId)).toEqual({
        provider_turn_id: "provider-turn-sanitized-context",
        selected_context_ids_json: '["memory-provider-approved"]',
        rejected_context_ids_json: JSON.stringify([
          "memory-provider-private",
          "memory-provider-unapproved",
          "memory-provider-injection",
        ]),
        exposed_payload_hash: expect.stringMatching(/^[a-f0-9]{64}$/),
        blocked: 0,
      });
    } finally {
      db.close();
