...
/** Discover one stable, secret-safe manifest per engagement-like top-level directory. */
export async function discoverLegacyEngagements(sourceRoots: readonly string[]): Promise<LegacyEngagementDiscovery> {
  ...
  for (const root of roots.roots) {
    ...
    for (const entry of entries) {
      ...
      const engagementKey = sha256Text(`${root.physicalIdentity}\0${entry.name.normalize("NFKC")}`);
      ...
      const modifiedAt = files.reduce((latest, file) => file.modifiedAt > latest ? file.modifiedAt : latest, statSync(engagementDirectory).mtime.toISOString());
      manifests.push({
        id: `legacy_engagement_${engagementKey.slice(0, 40)}`,
        root: root.canonicalPath,
        rootIdentity: root.physicalIdentity,
        engagementDirectory,
        engagementName: entry.name,
        engagementKey,
