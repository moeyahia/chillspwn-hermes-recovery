...
export class LegacyImporter {
  ...
  async import(): Promise<SourceMigrationResult> {
    ...
    switch (this.source.type) {
      ...
      case "hermes_state_sqlite": await this.importHermesState(stats); break;
    }
  ...
  private async importHermesState(stats: ImportStats): Promise<void> {
    const legacy = createDatabaseConnection({ filename: this.readPath, readonly: true, fileMustExist: true });
    try {
      ...
      if (!hasSessions || !hasMessages) {
        this.quarantine(stats, "database", this.source.sha256, "unsupported_hermes_schema", "sessions/messages tables not found");
        return;
      }
      ...
      for (const session of sessions) {
        ...
        const conversationId = stableId("conversation", this.sourceIdentity(), sessionKey);
        this.processItem(stats, `hermes-session:${sessionKey}`, session, () => {
