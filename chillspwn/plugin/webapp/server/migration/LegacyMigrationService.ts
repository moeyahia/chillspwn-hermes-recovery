...
export class LegacyMigrationService {
  ...
  async run(): Promise<LegacyMigrationResult> {
    ...
    if (!this.options.sourceRoots.length) throw new Error("At least one legacy source root is required");
    const rootResolution = canonicalizeLegacySourceRoots(this.options.sourceRoots);
    const sourceRoots = rootResolution.roots.map((root) => root.canonicalPath);
    ...
    const engagementDiscovery = await discoverLegacyEngagements(sourceRoots);
