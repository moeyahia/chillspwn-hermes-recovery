...
async function captureStableSource(source: LegacySource, destinationRoot: string): Promise<LegacySource> {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    if (
      (source.type === "kanban_sqlite" || source.type === "hermes_state_sqlite")
...
export class LegacyMigrationService {
  ...
  async run(): Promise<LegacyMigrationResult> {
    ...
    const sourceRoots = this.options.sourceRoots.map((root) => resolve(root));
    if (!sourceRoots.length) throw new Error("At least one legacy source root is required");
    ...
    const inventory = await discoverLegacySources(sourceRoots);
    ...
    const sqliteWalWarnings = inventory.included
      .filter((source) => source.type === "kanban_sqlite" || source.type === "hermes_state_sqlite")
    ...
    if (this.options.dryRun) {
      ...
      const report: ReconciliationReport = {
        ...
        databasePath,
        sourceRoots,
        counts: { sources: inventory.included.length, imported: 0, deduplicated: 0, quarantined: 0, skipped: 0 },
        targets: {},
        sources: inventory.included.map((source) => ({
    ...
    try {
      ...
      if (this.options.resumeMigrationId) {
        ...
        const discoveredPaths = new Set(inventory.included.map((source) => source.absolutePath));
...
export async function restoreMigrationBackup(options: RollbackOptions): Promise<void> {
  if (!options.serviceStopped) throw new Error("Rollback requires an explicit confirmation that the ChillsPwn service is stopped");
