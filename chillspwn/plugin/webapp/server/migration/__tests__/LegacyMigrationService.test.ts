...
function temporaryDirectory(): string {
  const path = mkdtempSync(join(tmpdir(), "chillspwn-legacy-migration-"));
...
function createHermesState(path: string): void {
  const database = createDatabaseConnection({ filename: path });
  try {
    database.exec(`
    ...
    // uncheckpointed WAL state. Make this fixture represent a quiescent Hermes
...
describe("LegacyMigrationService", () => {
  test("dry run hashes inventory and writes a report without touching canonical state", async () => {
    ...
    const databasePath = join(root, "command-os.sqlite");
  ...
  test("imports the Hermes session index without system prompts or secrets", async () => {
    const root = temporaryDirectory();
    const legacy = join(root, "legacy");
    const output = join(root, "migration-output");
    const databasePath = join(root, "command-os.sqlite");
    mkdirSync(legacy, { recursive: true });
    createCanonicalDatabase(databasePath);
    createHermesState(join(legacy, "state.db"));
    ...
    try {
      ...
      expect(body).not.toContain("hermes-secret");
