...
function actor(value: PlanChangeActor | undefined): PlanChangeActor {
  if (!value || !["operator", "reviewer", "admin"].includes(value.type)) {
    throw new PlanChangeError("plan_change_authentication_required", "Authenticated operator identity is required", "policy_denied", 401, "Sign in to the isolated Command OS V2 control plane.");
  }
