...
function bridge(mode: "disabled" | "dry-run" | "enabled", maxOut = 20000) {
  return new McpArsenalBridge({ configPath: cfgPath, manifestPath, mode, allowDocker: false, startServers: true, defaultTimeoutMs: 8000, maxOutputBytes: maxOut, trustedOwnerUid: process.geteuid?.() ?? 0 });
}
...
describe("16.1 registry: load + health states", () => {
  ...
  test("forAgent maps servers to specialists", () => { expect(registry().forAgent("ReconScout").map(s => s.name)).toContain("mock-recon"); });
});
