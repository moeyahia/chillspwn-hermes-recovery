...
function attackChainCategory(domain: string): import("../runtime/AttackLesson").TechniqueCategory {
...
export class RunLearningService {
  ...
  #calculate(
    ...
    const verifiedIds = new Set((this.database.prepare(`
      SELECT id FROM evidence WHERE run_id = ? AND verification_state = 'verified'
  ...
  #proposeLessons(input: {
    ...
    const verifiedEvidenceIds = (this.database.prepare(`
      SELECT id FROM evidence WHERE run_id = ? AND verification_state = 'verified' ORDER BY id
