...
function sendError(
  ...
  response.status(status).json({
    error: {
      ...
      category: status >= 500 ? "event_stream" : "invalid_input",
      traceId,
...
export function decodeSseEventId(value: string): string {
  ...
  if (!trimmed) throw new Error("Last-Event-ID is empty");
  if (!trimmed.startsWith("v2.")) {
    if (/[\x00\r\n]/u.test(trimmed)) throw new Error("Last-Event-ID is invalid");
    return trimmed;
  }
  try {
    ...
    throw new Error("Last-Event-ID is invalid");
  }
}
...
export function createEventStreamRouter(
  ...
  const replayHandler = (request: Request, response: Response): void => {
    ...
    try {
      ...
      if (decoded && queryRunId && queryRunId !== decoded.runId) {
        throw new Error("cursor does not belong to runId");
      }
      ...
      sendError(
        response,
        400,
        "invalid_event_replay_request",
        error instanceof Error ? error.message : "Invalid event replay request",
        traceId,
  ...
  router.get("/api/v2/events/stream", (request, response) => {
    ...
    try {
      ...
      if (!runId && afterSequence > 0) throw new Error("afterSequence requires runId");
      if (suppliedLastId) {
        ...
        if (runId) {
          ...
          if (!dependencies.service.eventExists(decodedId)) throw new Error("Last-Event-ID was not found");
      ...
      sendError(
        ...
        error instanceof Error ? error.message : "Invalid event stream request",
        traceId,
