...
export function describeRunMutationAuthorityError(
  ...
  if (error.code === "control_plane_mismatch") {
    return {
      ...
      retryable: false,
      remediation: "Open this run through its owning control plane; imported legacy runs remain read-only in Command OS V2.",
    };
  }
...
export class RunMutationAuthorityGuard {
  ...
  private ownedScope(runId: string): RunMutationScope {
    ...
    if (
      !mission
      || run.controlPlane !== "command_os_v2"
      || mission.controlPlane !== "command_os_v2"
    ) {
      throw new ControlPlaneLeaseError(
        "control_plane_mismatch",
        `Run ${runId} is not exclusively owned by Command OS V2`,
      );
    }
  ...
  authorize(input: {
    ...
    const resolveCurrentProof = (): ControlPlaneLease => {
      ...
      if (proof.runId !== scope.runId || proof.controlPlane !== "command_os_v2") {
        throw new ControlPlaneLeaseError(
          "lease_fence_invalid",
