...
export class RunRepository {
  ...
  authorizeAutonomousIntent(
    ...
    const destructivePolicy = typeof policy.destructivePolicy === "string"
      ...
      : "";
    if (intent.destructive && destructivePolicy !== "contract_only") {
    ...
    if (intent.kind === "tool") {
      ...
      const mapping = mappingFor(assignment.agent_id);
      if (
        !mapping?.allowedMcpServers.includes(mcpServer) ||
        specialistToolDecision(assignment.agent_id, toolName) !== "allow"
      ) {
  ...
  autonomousActionRemainsInContract(
    ...
    const destructivePolicy = typeof policy.destructivePolicy === "string"
      ? normalize(policy.destructivePolicy)
      : "";
    if (action.destructive && destructivePolicy !== "contract_only") return false;
    ...
    const mapping = mappingFor(currentAssignment.agent_id);
    return Boolean(mapping?.allowedMcpServers.includes(mcpServer))
      && specialistToolDecision(currentAssignment.agent_id, toolName) === "allow";
  }
